...
func SendStockInformationFlexFunc() {
	...
	*/
	rows, _ := db.Raw("SELECT a.token,c.* FROM line_users AS a JOIN line_user_stock AS b ON a.id = b.line_user_id JOIN stocks AS c ON b.stock_id = c.id ORDER BY a.id").Rows()
